...
class Filter:
  ...
  def __init__(self, params):
    self.curves = self._read_curves(params)
  ...
  def _filter_w_params(self, params):
    curves = []
    for param in params:
      curve = [(0,0)]
      # param[0] = (x1,y1)
      curve.append(param[0])
      curve.append(param[1])
      curve.append((255,255))
      curves.append(curve)
    curves.append([(255,255)])
    return curves

  def _read_curves(self, params):
    nr_curves = 5
    #curves = self._random_curve()
    curves = self._filter_w_params(params)
    return curves
...
def filter_image(params, image_array):
    
    # params is a list with length 4
    # each list contain 2 integers
    im = Image.fromarray(image_array)
    #im.save('original.png')
    #im.show()
    img_filter = Filter(params)
